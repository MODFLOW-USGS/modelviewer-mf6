...
{
    pCmdUI->SetCheck(!m_Manager->IsSolidVisible() && !m_Manager->AreIsosurfacesVisible() && m_Manager->GetDataFileList());
...
void CMvDoc::Animate()
{
    ...
    {
        while ((m_Manager->GetCurrentTimePointIndex() < m_Manager->GetNumberOfTimePoints() - 1) && m_IsAnimating)
        {
            start = clock();
            AdvanceOneTimePoint();
            finish   = clock();
    ...
    {
        m_IsAnimating = FALSE;
        m_AnimationDlg->m_ControlsPage->GetDlgItem(IDC_STOP_ANIMATION)->EnableWindow(FALSE);
    }
    if (m_AnimationType == atTime)
    {
        if (m_Manager->GetCurrentTimePointIndex() < m_Manager->GetNumberOfTimePoints() - 1)
        {
            m_AnimationDlg->m_ControlsPage->GetDlgItem(IDC_RUN_ANIMATION)->EnableWindow(TRUE);
            m_AnimationDlg->m_ControlsPage->GetDlgItem(IDC_ADVANCE_ANIMATION)->EnableWindow(TRUE);
        }
    }
    else
    {
    ...
    m_AnimationDlg->m_ControlsPage->GetDlgItem(IDC_SET_TO_TIME_POINT)->EnableWindow(TRUE);
    m_AnimationDlg->m_ControlsPage->GetDlgItem(IDC_TIME_POINT_LABELS)->EnableWindow(TRUE);
