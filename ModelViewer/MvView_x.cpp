...
//	m_Interactor = vtkWin32RenderWindowInteractor::New();
   ...
   m_Headlight->SetLightTypeToHeadlight();
	m_AuxiliaryLight = vtkSmartPointer<vtkLight>::New();
...
//	m_RenderWindow->Delete();
   if (m_vtkMFCWindow) delete m_vtkMFCWindow;
}
...
//	}
	...
	if (pDC->IsPrinting())
	{
		...
		BeginWaitCursor();
		memcpy(size,m_vtkMFCWindow->GetRenderWindow()->GetSize(),sizeof(int)*2);
		...
		pDC->DPtoLP(rcDestLP);
		int DPI = m_vtkMFCWindow->GetRenderWindow()->GetDPI();

		m_vtkMFCWindow->GetRenderWindow()->SetupMemoryRendering(rcDest.right/scale,
		...
		StretchBlt(pDC->GetSafeHdc(),0,0,
					...
					m_vtkMFCWindow->GetRenderWindow()->GetMemoryDC(),
					0, 0, rcDest.right/scale, rcDest.top/scale, SRCCOPY);

		m_vtkMFCWindow->GetRenderWindow()->ResumeScreenRendering();
		EndWaitCursor();
	}
	// Screen drawing code
	else
...
void CMvView::SetInteractorStyle(int interactorStyle)
{
	...
	vtkInteractorStyleSwitch *iss = vtkInteractorStyleSwitch::SafeDownCast(m_vtkMFCWindow->GetInteractor()->GetInteractorStyle());
	if (iss)
	{
...
{
	int *size = m_vtkMFCWindow->GetRenderWindow()->GetSize();
	...
	m_vtkMFCWindow->GetRenderWindow()->SetupMemoryRendering(width, height, GetDC()->GetSafeHdc());
	m_vtkMFCWindow->GetRenderWindow()->Render();
	file->write((char *) m_vtkMFCWindow->GetRenderWindow()->GetMemoryData(), dataWidth*height);
	if (magnification != 1)
	{
...
void CMvView::OnInitialUpdate() 
{
   ...
   m_vtkMFCWindow->GetRenderWindow()->AddRenderer(m_Renderer);

   vtkRenderWindowInteractor *in = m_vtkMFCWindow->GetInteractor();
	vtkInteractorStyleSwitch *iss = vtkInteractorStyleSwitch::SafeDownCast(in->GetInteractorStyle());
	iss->SetCurrentStyleToTrackballCamera();
...
void CMvView::OnCopyDisplayToClipboard() 
{
	...
	if (OpenClipboard())
	{
		...
		m_vtkMFCWindow->GetRenderWindow()->SetupMemoryRendering(size[0], size[1], GetDC()->GetSafeHdc());
		m_vtkMFCWindow->GetRenderWindow()->Render();

		memcpy((LPSTR)lpbi + lpbi->biSize, m_vtkMFCWindow->GetRenderWindow()->GetMemoryData(), 
					dataWidth*size[1]);
